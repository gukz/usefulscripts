...
if __name__ == "__main__":
    ...
    for s in services.split("\n"):
        ...
        if not s:
            continue
        #  service = stub.CreateService(octopus_pb2.CreateServiceReq(service_name=f"{s}.exam"))
        service = stub.GetService(octopus_pb2.GetServiceReq(service_name=f"{s}.exam"))
        print(service)
        res[s] = {
    ...
    json.dump(res, open("exam_svc_auth.json", "w"))
