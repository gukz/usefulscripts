...
/* BNF是最原始，最简单的方法，主要用于理论背景的学术论文中，以与人类进行交流为主。 */
/* 区别于(与编译器/解析器中使用相反)。BNF没有确切的规范。 */
/* EBNF表示扩展BNF，没有一个单一的EBNF，但是每个作者或程序都定义了自己的稍有不同的变体。 */
/* ABNF(增强型BNF)是与BNF完全不同的格式，但是更加标准化，它比较难阅读，但在解析器中最常用。 */
/* 总结一下，BNF表示适合教学，解释和理论探讨，比较简单。EBNF尤其是ABNF更常用于实际实现语法并由解析器读取 */
/* IETF 使用ABNF */
